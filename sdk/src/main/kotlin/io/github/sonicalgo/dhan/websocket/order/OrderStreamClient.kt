 ...
 * ```
 *
 * @see OrderStreamListener
 * @see <a href="https://dhanhq.co/docs/v2/order-update/">DhanHQ Live Order Updates</a>
 */
class OrderStreamClient internal constructor(
    dhanConfig: DhanConfig,
    wsConfig: DhanWebSocketConfig,
    wsHttpClient: OkHttpClient
) : BaseWebSocketClient(wsHttpClient, dhanConfig, wsConfig, "OrderStream") {

    private val listeners = CopyOnWriteArrayList<OrderStreamListener>()
     ...
     */
    fun addListener(listener: OrderStreamListener) {
     ...
     */
    fun removeListener(listener: OrderStreamListener) {
    ...
    override fun onWebSocketMessage(text: String) {
        try {
            ...
            when {
                // Connection status message - handle internally
                jsonObject.containsKey("status") || jsonObject.containsKey("type") -> {
                    ...
                    // Notify via onError only on auth failure
                    if (!status.isSuccess) {
                        val errorMsg = status.message ?: "Authentication failed"
                        notifyListeners { it.onError(RuntimeException("Connection status: $errorMsg")) }
                    }
                    // Auth success - no notification needed (onConnected already called)
                }
     ...
     */
    private inline fun notifyListeners(action: (OrderStreamListener) -> Unit) {

 ...
 */
class DhanMarketFeedClient internal constructor(
    dhanConfig: DhanConfig,
    wsConfig: DhanWebSocketConfig,
    wsHttpClient: OkHttpClient
) : BaseWebSocketClient(wsHttpClient, dhanConfig, wsConfig, "DhanMarketFeed") {
        ...
        if (subscriptions.size + newInstruments.size > DhanConstants.WEBSOCKET_MAX_INSTRUMENTS_PER_CONNECTION) {
            listeners.forEach {
                it.onError(IllegalStateException(
                    "Maximum instruments per connection (${DhanConstants.WEBSOCKET_MAX_INSTRUMENTS_PER_CONNECTION}) exceeded"
                ))
            }
            return false
        }
    ...
    private inline fun notifyListeners(action: (MarketFeedListener) -> Unit) {
        listeners.forEach { listener ->
            try {
                ...
                System.err.println("DhanMarketFeedClient: Listener threw exception: ${e.message}")
            }
        }
    }
